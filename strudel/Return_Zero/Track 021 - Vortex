...
i: s("hh!3").lpf(sine.range(3000, 5000)).room(.5).pan(0.7)
n: s("<rim:0 rim:2>!12").gain(rand.rib(32,2).sub(1)).clip(.3).lpf(2000).pan("<0.1 0.2 0 0.8 0.6>")
...
raincloud: chord(chords).s("piano").voicing()
  .distort("1:.4").pan(.4).jux(rev)
  .scope()

flood: chord(chords).seg(1).s("gm_violin").voicing()
  .distort("2:.17").lpf(400).lpenv(2).lpq(15)
  .scope()

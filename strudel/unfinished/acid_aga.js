...
let acid_bass = n("{-7 0@2 2 7 <0 0> <0 - > 0}*2".add("-7"))
  .scale("e:minor").s("saw").att(sine.range(0.02, .1))
...
acid_layer_sub: acid_bass  
  .off(1/16, x=>x.s("supersaw")).velocity(.2)

let kick = s("bd bd:1").struct("1 1 1 1 1 1 1 1").room(.3)
  .every(4, x=>x.ply(2)).velocity(.8)
let snare = s("sd".struct("0 1 0 1")).room(.3).delay(.5)
...
drums: stack(kick, snare, clap, hat)
